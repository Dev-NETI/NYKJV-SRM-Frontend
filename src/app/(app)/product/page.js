...
import { useProducts } from '@/hooks/api/product';
import { useBrand } from '@/hooks/api/brand';
import { useCategory } from '@/hooks/api/category';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { Box, Container, Paper, Button, Dialog, DialogTitle, DialogContent, DialogActions, Grid2, Typography } from '@mui/material';
import { DataGrid } from '@mui/x-data-grid';

const ProductComponent = () => {
  const { index: showProduct, store, update: updateProduct, deleteProduct } = useProducts();
  ...
  const columns = [
    ...
    { field: 'specification', headerName: 'Specification', flex: 1, minWidth: 140 },
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  <Grid2 container spacing={2}>
                    <Grid2 item size={{ xs: 8}}>
                      <Typography variant="body1" color="textSecondary"><strong>Product Name:</strong></Typography>
                    ...
                    <Grid2 item size={{ xs: 4}}>
                      <Typography variant="body1" color="textSecondary"><strong>Price:</strong></Typography>
                      <item sx={{ fontSize: '1.1rem' }}>
                        <input
                          type="text"
                          id="productPrice"
            ...
            {viewProduct && (
              <Box p={2}>
                <Grid2 container spacing={2}>
                  <Grid2 item size={{ xs: 12}}>
                    <Typography variant="body1" color="textSecondary"><strong>Product Name:</strong></Typography>
                    <item sx={{ fontSize: '1.1rem' }}>{viewProduct.name || "N/A"}</item>
                  </Grid2>
                  <Grid2 item size={{ xs: 6}}>
                    <Typography variant="body1" color="textSecondary"><strong>Category:</strong></Typography>
                    <item sx={{ fontSize: '1.1rem' }}>{viewProduct.category_id || "N/A"}</item>
                  </Grid2>
                  <Grid2 item size={{ xs: 6}}>
                    <Typography variant="body1" color="textSecondary"><strong>Brand:</strong></Typography>
                    <item sx={{ fontSize: '1.1rem' }}>{viewProduct.brand_id || "N/A"}</item>
                  </Grid2>
                  <Grid2 item size={{ xs: 6}}>
                    <Typography variant="body1" color="textSecondary"><strong>Price:</strong></Typography>
                    <item sx={{ fontSize: '1.1rem' }}>{viewProduct.price || "N/A"}</item>
                  </Grid2>
                  <Grid2 item size={{ xs: 6}}>
                    <Typography variant="body1" color="textSecondary"><strong>Specification:</strong></Typography>
